...
if ($nv_Request->isset_request('checkss', 'post')) {
  ...
  if ($nv_Request->isset_request('loadForm', 'post')) {
    $load_form = $nv_Request->get_title('loadForm', 'post', '');
    $array_content = array(
      'fname' => $fname,
      'femail' => $femail,
      'fphone' => $fphone,
    ...
    $form = contact_formset($array_content, $load_form, $catsName, $base_url, NV_CHECK_SESSION);
  ...
  $car_register = nv_substr($nv_Request->get_title('car_register', 'post', '', 1), 0, 100);
  $appointment_date = nv_substr($nv_Request->get_title('appointment_date', 'post', '', 1), 0, 100);

  $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_send
    (cid, cat, title, car_register, appointment_date, content, send_time, sender_id, sender_name, sender_email, sender_phone, sender_address, sender_ip, is_read, is_reply) VALUES
    (' . $fpart . ', :cat, :title, :car_register, :appointment_date, :content, ' . NV_CURRENTTIME . ', ' . $sender_id . ', :sender_name, :sender_email, :sender_phone, :sender_address, :sender_ip, 0, 0)';
  $data_insert = array();
  $data_insert['cat'] = $fcat;
  $data_insert['title'] = $ftitle;
  $data_insert['car_register'] = $car_register;
  $data_insert['appointment_date'] = $appointment_date;
  ...
  if ($row_id > 0) {
    $fcon_mail = contact_sendcontact($row_id, $fcat, $ftitle, $fname, $femail, $fphone, $fcon, $fpart, $faddress, $car_register, $appointment_date);
    ...
    if ($fsendcopy) {
      ...
      $fcon_mail = contact_sendcontact($row_id, $fcat, $ftitle, $fname, $femail, $fphone, $fcon, $fpart,$faddress, $car_register, $appointment_date, false);
