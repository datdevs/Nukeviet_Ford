...
if (!nv_function_exists('nv_block_slider')) {
    ...
    {
        global $global_config, $site_mods, $module_config, $nv_Cache, $db, $client_info;
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $row) {
                if (!empty($row['image'])) {
                    $row['image'] = $client_info['is_mobile'] ? NV_BASE_SITEURL . NV_FILES_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $row['image'] : NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $row['image'];
                }
