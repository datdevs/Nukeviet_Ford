...
if (!nv_function_exists('nv_block_news_cat_middle')) {
    /**
     * nv_block_config_news_cat_middle()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_news_cat_middle($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_news_cat_middle_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_news_cat_middle_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_block_news_cat_middle()
     *
     * @param mixed $block_config
     * @return
     */
    function nv_block_news_cat_middle($block_config)
    {
        ...
        $db->sqlreset()
            ->select('id, catid, title, alias, homeimgfile, homeimgthumb, hometext, publtime, external_link, hitstotal')
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_grid_item.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_grid_item.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
            ...
            foreach ($list as $l) {
                ...
                if ($l['homeimgthumb'] == 1) {
                    $l['imgurl'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'];
                } elseif ($l['homeimgthumb'] == 2) {
                    $l['imgurl'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'];
                } elseif ($l['homeimgthumb'] == 3) {
                    $l['imgurl'] = $l['homeimgfile'];
                } elseif (!empty($show_no_image)) {
                    $l['imgurl'] = NV_BASE_SITEURL . $show_no_image;
                } else {
                    $l['imgurl'] = '';
                }
                ...
                $l['title_clean'] = nv_clean60($l['title'], 40);

                $l['publishdate'] = nv_RelativeTime($l['publtime'], 'd/m/Y');
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_news_cat_middle($block_config);
    }
}