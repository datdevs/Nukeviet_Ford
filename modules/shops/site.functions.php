...
foreach ($global_array_group as $key => $array_group) {
  if ($array_group['lev'] == 0) {
    $array_subgroupid = explode(',', $array_group['subgroupid']);
    $global_array_group[$key]['listgroup'] = array();
    foreach ($global_array_group as $array_sub_group) {
      if (in_array($array_sub_group['groupid'], $array_subgroupid)) {
        array_push($global_array_group[$key]['listgroup'], $array_sub_group);
      }
    }
  }
}
