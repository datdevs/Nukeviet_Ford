...
function detail_product($data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_keyword)
{
  ...
  if (!empty($data_content)) {
    ...
    if (!empty($data_content['prod_image_ver'])) {
      $each_prod_image = explode('|', $data_content['prod_image_ver']);
      foreach ($each_prod_image as $each_prod_image_i) {
        if (!empty($each_prod_image_i) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $each_prod_image_i)) {
          $each_prod_image_full = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $each_prod_image_i;
          $each_prod_image_thumb = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $module_upload . '/' . $each_prod_image_i;
          $xtpl->assign('ALT', $each_prod_image_i);
          $xtpl->assign('IMG_FULL', $each_prod_image_full);
          $xtpl->assign('IMG_THUMBS', $each_prod_image_thumb);
          $xtpl->parse('main.prod_image_ver_full.loop');
          $xtpl->parse('main.prod_image_ver_thumbs.loop');
        }
      }
      $xtpl->parse('main.prod_image_ver_full');
      $xtpl->parse('main.prod_image_ver_thumbs');
    }
    ...
    $site_description = preg_replace(['/<[^>]*>/', '/[\r\n\t]+/'], ' ', $data_content[NV_LANG_DATA . '_hometext']);
    $site_description = trim(preg_replace('/[ ]+/', ' ', $site_description));
    $xtpl->assign('site_description', $site_description);
      ...
      if (!empty($data_content['tabs'])) {
        ...
        foreach ($data_content['tabs'] as $tabs_id => $tabs_value) {
          ...
          if ($tabs_key == 'content_detail') {
            ...
            $tabs_content = '<div class="fnt-product-details">' . $data_content[NV_LANG_DATA . '_bodytext'] . "</div>";
          } elseif ($tabs_key == 'content_otherimage') {
            // Hình ảnh khác

            $tabs_content = nv_display_othersimage($data_content['otherimage']);
          } elseif ($tabs_key == 'content_specs') {
            // Thông số kỹ thuật

            $tabs_content = '<div class="fnt-product-specifications">' . $data_content[NV_LANG_DATA . '_specs'] . "</div>";
...
function compare($data_pro)
{
  ...
  foreach ($data_pro as $data_row) {
    ...
    $xtpl->assign('intro', $data_row['hometext']);
    $xtpl->parse('main.hometext');
    $xtpl->assign('specs', $data_row['specs']);
    $xtpl->parse('main.specs');
    // if (!empty($data_row['specs'])) {
    //   $xtpl->assign('specs', $data_row['specs']);
    //   $xtpl->parse('main.specs');
    // }
    // else {
    //   $xtpl->assign('bodytext', $data_row['bodytext']);
...
function wishlist($data_content, $compare_id, $html_pages = '')
{
  ...
  $xtpl->assign('CSS_PRODUCT_CODE', !empty($pro_config['show_product_code']) ? ' show-product-code' : '');
  $xtpl->assign('num', 12 / $pro_config['per_row']);
...
function nv_display_othersimage($otherimage)
{
  ...
  $xtpl->assign('LINK', NV_BASE_SITEURL . 'themes/' . $module_info['template']);
