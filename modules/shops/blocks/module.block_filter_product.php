...
if (!function_exists('nv_filter_product')) {
  ...
  function nv_filter_product($block_config)
  {
    ...
    foreach ($global_array_group as $arr_group) {
      ...
      if ($arr_group['lev'] == 0 && $arr_group['inhome']) {
        $xtpl->assign('GROUP', array('title' => $arr_group['title'], 'image' => $arr_group['image']));

        foreach ($arr_group['listgroup'] as $sub_arr_group) {
          if ($sub_arr_group['inhome']) {
            $xtpl->assign('SUBGROUP', array('id' => $sub_arr_group['groupid'], 'title' => $sub_arr_group['title'], 'numpro' => $sub_arr_group['numpro'], 'image' => $sub_arr_group['image'], 'checked' => in_array($sub_arr_group['groupid'], $array_id) ? 'checked="checked"' : ''));
            $xtpl->parse('main.group.listgroup.checkbox');
            $xtpl->parse('main.group.listgroup');
          }
        }
      ...
      //   foreach ($global_array_group as $sub_arr_group) {
