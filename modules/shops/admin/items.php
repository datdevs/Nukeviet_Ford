...
// Cập nhật lại order_num sản phẩm 
// Sử dụng xong thì comment
// $total = $db->query('SELECT id, order_num FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows');
// $total_found = $db->query('SELECT FOUND_ROWS()');
// $order_num = $total_found->fetchColumn();
// foreach ($total as $key => $row) {
//     $stmt = $db->prepare("UPDATE " . $db_config['prefix'] . "_" . $module_data . "_rows SET order_num=" . intval($key) . " WHERE id=" . $row['id']);
//     $stmt->execute();
// }
...
if ($data['sortdefault'] == 3 && $checkss == '' && $page == 1) {
    $xtpl->parse('main.sortable');
}
...
if ($data['sortdefault'] == 3) {
    $ord_sql = 'order_num ASC';
}
$db->sqlreset()->select('id, listcatid, order_num, user_id, homeimgfile, homeimgthumb, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, hitstotal, status, edittime, publtime, exptime, product_number, product_price, money_unit, product_unit, num_sell, username')->from($from)->order($ord_sql)->limit($per_page)->offset(($page - 1) * $per_page);
$result = $db->query($db->sql());

$theme = $site_mods[$module_name]['theme'] ? $site_mods[$module_name]['theme'] : $global_config['site_theme'];
$a = 0;

while (list($id, $listcatid, $order_num, $admin_id, $homeimgfile, $homeimgthumb, $title, $alias, $hitstotal, $status, $edittime, $publtime, $exptime, $product_number, $product_price, $money_unit, $product_unit, $num_sell, $username) = $result->fetch(3)) {
