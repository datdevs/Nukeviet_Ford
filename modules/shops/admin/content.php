...
$order_num = $db->query('SELECT COUNT(*) FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows')->fetchColumn();
...
if ($nv_Request->get_int('save', 'post') == 1) {
  ...
  $specs = $nv_Request->get_string('specs', 'post', '');
  $rowcontent['specs'] = defined('NV_EDITOR') ? nv_nl2br($specs, '') : nv_nl2br(nv_htmlspecialchars(strip_tags($specs)), '<br />');
  ...
  $prod_image_ver = $nv_Request->get_typed_array('prod_image_ver', 'post', 'string');
  $array_prod_image_ver = array();
  foreach ($prod_image_ver as $prod_image_ver_i) {
    if (!nv_is_url($prod_image_ver_i) and file_exists(NV_DOCUMENT_ROOT . $prod_image_ver_i)) {
      $lu = strlen(NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/');
      $prod_image_ver_i = substr($prod_image_ver_i, $lu);
    } elseif (!nv_is_url($prod_image_ver_i)) {
      $prod_image_ver_i = '';
    }
    if (!empty($prod_image_ver_i)) {
      $array_prod_image_ver[] = $prod_image_ver_i;
    }
  }
  $rowcontent['prod_image_ver'] = implode('|', $array_prod_image_ver);
  ...
  if (empty($error)) {
    ...
    if ($rowcontent['id'] == 0 or $is_copy) {
      ...
      $sql = "INSERT INTO " . $db_config['prefix'] . "_" . $module_data . "_rows (id, listcatid, order_num, user_id, addtime, edittime, status, publtime, exptime, archive, product_code, product_number, product_price, price_config, money_unit, product_unit, product_weight, weight_unit, discount_id, homeimgfile, homeimgthumb, homeimgalt,otherimage, prod_image_ver, imgposition, copyright, inhome, allowed_comm, allowed_rating, ratingdetail, allowed_send, allowed_print, allowed_save, hitstotal, hitscm, hitslm, showprice " . $listfield . ")
				 VALUES ( NULL ,
				 :listcatid,
         " . intval($rowcontent['order_num']) . ",
      ...
      $data_insert['prod_image_ver'] = $rowcontent['prod_image_ver'];
      ...
      $rowcontent['order_num'] = $rowcontent_old['order_num'];
      ...
      $stmt = $db->prepare("UPDATE " . $db_config['prefix'] . "_" . $module_data . "_rows SET
			 listcatid= :listcatid,
       order_num=" . intval($rowcontent['order_num']) . ",
      ...
      $stmt->bindParam(':prod_image_ver', $rowcontent['prod_image_ver'], PDO::PARAM_STR);
      ...
      $stmt->bindParam(':specs', $rowcontent['specs'], PDO::PARAM_STR, strlen($rowcontent['specs']));
  ...
  $rowcontent['specs'] = $rowcontent[NV_LANG_DATA . '_specs'];
...
if (!empty($rowcontent['prod_image_ver'])) {
  $prod_image_ver = explode('|', $rowcontent['prod_image_ver']);
} else {
  $prod_image_ver = array();
}
...
$prod_items = 0;
if (!empty($prod_image_ver)) {
  foreach ($prod_image_ver as $prod_image_ver_i) {
    if (!empty($prod_image_ver_i) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $prod_image_ver_i)) {
      $prod_image_ver_i = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $prod_image_ver_i;
    }
    $data_prod_image_ver = array(
      'id' => $prod_items,
      'value' => $prod_image_ver_i
    );
    $xtpl->assign('PROD_VER', $data_prod_image_ver);
    $xtpl->parse('main.prod_ver');
    ++$prod_items;
  }
}
$xtpl->assign('PROD_FILE_ITEMS', $prod_items);
...
$rowcontent['specs'] = htmlspecialchars(nv_editor_br2nl($rowcontent['specs']));
if (defined('NV_EDITOR') and function_exists('nv_aleditor')) {
  $edits = nv_aleditor('specs', '100%', '300px', $rowcontent['specs']);
} else {
  $edits = "<textarea style=\"width: 100%\" name=\"specs\" id=\"specs\" cols=\"20\" rows=\"15\">" . $rowcontent['specs'] . "</textarea>";
}
$xtpl->assign('edit_specs', $edits);
