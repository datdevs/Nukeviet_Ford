...
function insertvaluetofield() {
  ...
  var imageInfo = $("img[title='" + selFile + "']")
    .attr('name')
    .split('|')
...
function download() {
  var selFile = $('input[name=selFile]').val()
  var selFileData = $("img[title='" + selFile + "']")
    .attr('name')
    .split('|')
  ...
  if (selFileData[3] == 'image' || selFileData[2] == 'swf') {
    ...
    if (selFileData[3] == 'image') {
      ...
      $('div#fileView img').click(function () {
        ...
        $('#sitemodal')
          .find('.modal-body')
          .html('<div class="text-center"><img class="img-responsive" src="' + nv_base_siteurl + fullPath + '/' + selFile + '?' + selFileData[8] + '" /></div>')
        $('#sitemodal').modal()
      })
    }
  } else {
...
function create() {
  ...
  if (selFileData[3] == 'image') {
    ...
    $('img[name=myFile2]')
      .width(DisSize[0])
      .height(DisSize[1])
      .attr('src', nv_base_siteurl + path + '/' + selFile + '?' + selFileData[8])
...
function move() {
  ...
  if (selFile.indexOf('|') == -1) {
    ...
    var selFileData = $("img[title='" + selFile[0] + "']")
      .attr('name')
      .split('|')
    var path = selFileData[7] == '' ? $('span#foldervalue').attr('title') : selFileData[7]
    ...
    $.ajax({
      ...
      success: function (e) {
        ...
        if (e[0] != 'OK') {
          ...
          $('#imglist')
            .html(nv_loading_data)
            .load(nv_module_url + 'imglist&path=' + path + '&type=' + imgtype + author + '&order=' + order + '&num=' + +nv_randomNum(10), function () {
              LFILE.setViewMode()
            })
        }
      },
  ...
  if (e.which != 3) {
    ...
    if (folderPath != $('span#foldervalue').attr('title')) {
      ...
      if ($(folder).is('.view_dir')) {
        ...
        $('div#imglist')
          .html(nv_loading_data)
          .load(nv_module_url + 'imglist&path=' + folderPath + '&imgfile=' + selFile + '&type=' + imgtype + author + '&order=' + $('select[name=order]').val() + '&random=' + nv_randomNum(10), function () {
            LFILE.setViewMode()
          })
      } else {
        $('div#imglist').text('')
...
function searchfile() {
  ...
  $('#imglist')
    .html(nv_loading_data)
    .load(nv_module_url + 'imglist&path=' + a + '&type=' + b + e + '&q=' + rawurlencode(q) + '&order=' + $('select[name=order]').val() + '&random=' + nv_randomNum(10), function () {
      LFILE.setViewMode()
    })
  return false
}
...
function cropfile() {
  ...
  if (selFileData[0] < 10 || selFileData[1] < 10 || (selFileData[0] < 16 && selFileData[1] < 16)) {
    ...
    $('#cropButtons').html('<div class="margin-left text-left"><label><input type="checkbox" id="crop-keeporg" value="1"/>' + LANG.crop_keep_original + '</label></div>' + 'X:<input type="text" id="crop-x" value="" class="w50 form-control" readonly="readonly"/> ' + 'Y:<input type="text" id="crop-y" value="" class="w50 form-control" readonly="readonly"/> ' + 'W:<input type="text" id="crop-w" value="" class="w50 form-control" readonly="readonly"/> ' + 'H:<input type="text" id="crop-h" value="" class="w50 form-control" readonly="readonly"/> ' + '<input type="button" id="crop-save" value="' + LANG.save + '" class="btn btn-primary"/>')
...
function addlogo() {
  ...
  if (selFileData[0] < 10 || selFileData[1] < 10 || (selFileData[0] < 16 && selFileData[1] < 16)) {
    ...
    $('#addlogoButtons').html('X:<input type="text" id="addlogo-x" value="" class="w50 form-control" readonly="readonly"/> ' + 'Y:<input type="text" id="addlogo-y" value="" class="w50 form-control" readonly="readonly"/> ' + 'W:<input type="text" id="addlogo-w" value="" class="w50 form-control" readonly="readonly"/> ' + 'H:<input type="text" id="addlogo-h" value="" class="w50 form-control" readonly="readonly"/> ' + '<input type="button" id="addlogo-save" value="' + LANG.save + '" class="btn btn-primary"/>')
...
$('.vchange').change(function () {
  ...
  $('#imglist')
    .html(nv_loading_data)
    .load(nv_module_url + 'imglist&path=' + a + '&type=' + d + '&imgfile=' + b + e + '&order=' + $('select[name=order]').val() + '&random=' + nv_randomNum(10), function () {
      LFILE.setViewMode()
    })
})
...
$('.refresh em').click(function () {
  ...
  $('#imgfolder')
    .html(nv_loading_data)
    .load(nv_module_url + 'folderlist&path=' + g + '&currentpath=' + a + '&dirListRefresh&random=' + nv_randomNum(10), function () {
      loaded++
  ...
  $('#imglist')
    .html(nv_loading_data)
    .load(nv_module_url + 'imglist&path=' + a + '&type=' + b + '&imgfile=' + d + e + '&refresh&order=' + $('select[name=order]').val() + '&random=' + nv_randomNum(10), function () {
      loaded++
...
var LFILE = {
  reload: function (path, file) {
    ...
    $('#imglist')
      .html(nv_loading_data)
      .load(nv_module_url + 'imglist&path=' + path + '&type=' + imgtype + '&imgfile=' + file + author + '&order=' + order + '&num=' + nv_randomNum(10), function () {
        LFILE.setViewMode()
      })
  },
  setSelFile: function () {
...
var NVUPLOAD = {
  ...
  started: false,
  buttons: '<div class="row">' + '<div class="col-sm-14 buttons">' + '<div class="btn-group dropup browse-button open perload" id="upload-dropdown-btn">' + '<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="true">' + LANG.upload_mode + ' <span class="caret"></span>' + '</button>' + '<ul class="dropdown-menu" role="menu">' + '<li><a id="upload-remote" onclick="return remoteUpload();" href="#">' + LANG.upload_mode_remote + '</a></li>' + '<li><a id="upload-local" href="#">' + LANG.upload_mode_local + '</a></li>' + '</ul>' + '</div>    ' + '</div>' + '<div class="col-sm-10">' + '<div class="row" id="upload-queue-total">' + '<div class="col-sm-8 total-size"></div>' + '<div class="col-sm-16 total-status"></div>' + '</div>' + '</div>' + '</div>',
      ...
      NVUPLOAD.uploader = new plupload.Uploader({
        ...
        init: {
            ...
            if (NVUPLOAD.uploader.state === plupload.STARTED) {
              if (!NVUPLOAD.started) {
                ...
                if (parseFloat(nv_chunk_size) <= 0) {
                  $('#upload-button-area .buttons').append('<input id="upload-stop" type="button" class="btn btn-primary" value="' + LANG.upload_stop + '"/> ' + '<input style="display:none" id="upload-continue" type="button" class="btn btn-primary" value="' + LANG.upload_continue + '"/>')
                }

                $('#upload-button-area .buttons').append('<div class="total-info pull-right"></div>')
                $('#upload-button-area .total-info').html(plupload.sprintf(LANG.upload_info, NVUPLOAD.uploader.total.uploaded, NVUPLOAD.uploader.files.length, plupload.formatSize(NVUPLOAD.uploader.total.bytesPerSec)))

                // Init upload progress bar
                $('#upload-queue-total .total-status').html('<div class="progress">' + '<div class="progress-bar" role="progressbar" aria-valuenow="' + NVUPLOAD.uploader.total.percent + '" aria-valuemin="0" aria-valuemax="100" style="width: ' + NVUPLOAD.uploader.total.percent + '%;">' + NVUPLOAD.uploader.total.percent + '%</div>' + '</div>')
          ...
          Error: function (up, err) {
            ...
            $('div#errorInfo')
              .html('Error #' + err.message + ': <br>' + err.file.name)
              .dialog('open')
  ...
  renderUI: function () {
    ...
    // Add some button
    $('#upload-button-area .buttons').append('<button id="upload-start" type="button" class="btn btn-primary">' + LANG.upload_file + '</button> ' + '<input id="upload-cancel" type="button" class="btn btn-default" value="' + LANG.upload_cancel + '"/> ')
    ...
    // Build upload queue
    $('#upload-queue').html('<div class="queue-header">' + '<div class="container-fluid">' + '<div class="row">' + '<div class="col-sm-' + (nv_alt_require ? '8' : '14') + '">' + LANG.file_name + '</div>' + (nv_alt_require ? '<div class="col-sm-6">' + LANG.altimage + '</div>' : '') + '<div class="col-sm-4">' + LANG.upload_size + '</div>' + '<div class="col-sm-6">' + LANG.upload_status + '</div>' + '</div>' + '</div>' + '</div>' + '<div id="upload-queue-files" class="container-fluid"></div>\
  ...
  updateList: function () {
    ...
    $.each(NVUPLOAD.uploader.files, function (i, file) {
      if (!nv_alt_require) {
        fileList.append('<div id="' + file.id + '" class="row file-item">' + '<div class="col-sm-14 file-name"><span>' + file.name + '</span></div>' + '<div class="col-sm-4 file-size">' + plupload.formatSize(file.size) + '</div>' + '<div class="col-sm-4 file-status">' + file.percent + '%</div>' + '<div class="col-sm-2 file-action text-right"></div>' + '</div>')
      } else {
        ...
        fileList.append('<div id="' + file.id + '" class="row file-item">' + '<div class="col-sm-8 file-name"><span>' + file.name + '</span></div>' + '<div class="col-sm-6 file-alt"><input type="text" value="' + fileAlt + '" onkeyup="NVLDATA.setValue( \'' + file.id + '\', this.value);" class="form-control upload-file-alt dynamic"/></div>' + '<div class="col-sm-4 file-size">' + plupload.formatSize(file.size) + '</div>' + '<div class="col-sm-4 file-status">' + file.percent + '%</div>' + '<div class="col-sm-2 file-action text-right"></div>' + '</div>')
      }
...
var NVCMENU = {
  ...
  init: function () {
    NVCMENU.menu = $('<div id="nvContextMenu"></div>')
      .hide()
      .css({
        position: 'absolute',
        zIndex: '500',
    ...
    NVCMENU.shadow = $('<div id="nvContextMenuShadow"></div>')
      .hide()
      .css({
        backgroundColor: '#000',
        position: 'absolute',
  ...
  show: function (e) {
    ...
    if ($('#contextMenu').html() != '') {
      ...
      content
        .css(NVCMENU.menuStyle)
        .find('li')
        .css(NVCMENU.itemStyle)
        .hover(
          function () {
