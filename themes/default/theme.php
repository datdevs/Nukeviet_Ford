...
function nv_site_theme($contents, $full = true)
{
  ...
  if ($module_info['module_theme'] == 'home') {
    $xtpl->assign('script_src', NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/app/build/js/i.js');
    $xtpl->parse('main.script');
    $xtpl->parse('main.styles.i');
  } elseif ($module_info['module_theme'] == 'contact') {
    $xtpl->assign('script_src', NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/app/build/js/c.js');
    $xtpl->parse('main.script');
    $xtpl->parse('main.styles.c');
  } elseif ($module_info['module_theme'] == 'news') {
    $xtpl->parse('main.styles.n');
  } elseif ($module_info['module_theme'] == 'page') {
    $xtpl->parse('main.styles.p');
  } elseif ($module_info['module_theme'] == 'shops') {
    $xtpl->assign('script_src', NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/app/build/js/s.js');
    $xtpl->parse('main.script');
    $xtpl->parse('main.styles.s');
  }
  $xtpl->parse('main.styles');

  $html_js = nv_html_site_js(false);
  $html_js[] = [
    'async' => 0,
    'ext' => 1,
    'content' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/js/function.js'
  ];
  $html_js[] = [
    'async' => 1,
    'ext' => 1,
    'content' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/app/build/js/g.js'
  ];
