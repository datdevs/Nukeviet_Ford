...
function nv_sh(sl_id, div_id) {
  ...
  if (new_opt == 3) nv_show_hidden(div_id, 1)
  else nv_show_hidden(div_id, 0)
  return false
}
...
function nv_add_otherprodimage() {
  //	var newitem = "<div class=\"form-group\"><input class=\"form-control\" value=\"\" name=\"otherimage[]\" id=\"otherimage_" + file_items + "\" style=\"width : 80%\" maxlength=\"255\" />";
  //newitem += "&nbsp;<input type=\"button\" class=\"btn btn-info\" value=\"" + file_selectfile + "\" name=\"selectfile\" onclick=\"nv_open_browse( '" + nv_base_adminurl + "index.php?" + nv_name_variable + "=upload&popup=1&area=otherimage_" + file_items + "&path=" + file_dir + "&type=file&currentpath=" + currentpath + "', 'NVImg', 850, 400, 'resizable=no,scrollbars=no,toolbar=no,location=no,status=no' ); return false; \" /></div>";
  ...
  newitem += '<div class="form-group">'
  newitem += '<div class="input-group">'
  newitem += '	<input class="form-control" type="text" name="prod_image_ver[]" id="prod_image_ver_' + prod_file_items + '" />'
  newitem += '	<span class="input-group-btn">'
  newitem += '		<button class="btn btn-default" type="button" onclick="nv_open_browse( \'' + nv_base_adminurl + 'index.php?' + nv_name_variable + '=upload&popup=1&area=prod_image_ver_' + prod_file_items + '&path=' + file_dir + '&type=file&currentpath=' + currentpath + "', 'NVImg', 850, 400, 'resizable=no,scrollbars=no,toolbar=no,location=no,status=no' ); return false; \" >"
  newitem += '			<em class="fa fa-folder-open-o fa-fix">&nbsp;</em></button>'
  newitem += '	</span>'
  newitem += '</div>'
  newitem += '</div>'

  $('#prod_more_image_ver').append(newitem)
  prod_file_items++
}
...
function GetNumber(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (temp == '.') {
      if (count > 0) return str.substring(0, ipubl_date)
      count++
    }
  }
  return str
