...
<div class="well">
	<form action="{NV_BASE_ADMINURL}index.php" method="get">
		...
		<em class="help-block" style="margin:0"><div><i class="fa fa-info-circle" aria-hidden="true"></i> Nhấn và giữ vào hàng để sắp xếp sản phẩm</div>
		<div><i class="fa fa-info-circle" aria-hidden="true"></i> Khi tìm kiếm sản phẩm, tính năng sắp xếp sẽ được tắt đi</div></em>
	</form>
</div>
...
<form class="form-inline" name="block_list">
	<div class="table-responsive">
		<table class="table table-striped table-bordered table-hover">
			...
			<tbody id="nv_sortable">
				<!-- BEGIN: loop -->
				<tr data-order-id="{ROW.id}" data-order-num="{ROW.order_num}">
...
<script type="text/javascript" src="{NV_BASE_SITEURL}{NV_ASSETS_DIR}/js/sortable/i.js"></script>
<script type='text/javascript'>
var nv_sortable=document.getElementById("nv_sortable");new Sortable(nv_sortable,{animation:150,dataIdAttr:"data-order-id",ghostClass:"nv_sortable_ghost",onEnd:function(a){var e=this.toArray();e.forEach((a,e)=>{document.querySelector('[data-order-id="'+a+'"]').dataset.orderNum=e}),$.ajax({type:"POST",url:script_name+"?"+nv_name_variable+"="+nv_module_name+"&"+nv_fc_variable+"=update_items_order",data:{order_id:e},cache:!1,success:function(a){console.log(a)}})}});
</script>
<!-- END: sortable -->
<!-- END: main -->
...
<!--
<script type='text/javascript'>
	var nv_sortable = document.getElementById('nv_sortable');
	new Sortable(nv_sortable, {
		animation: 150,
		dataIdAttr: 'data-order-id',
		ghostClass: 'nv_sortable_ghost',
		onEnd: function (evt) {
			var order_id = this.toArray();
			order_id.forEach((id, index) => {
				var el = document.querySelector('[data-order-id="' + id + '"]')
				el.dataset.orderNum = index
			});
			$.ajax({
				type: "POST",
				url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=update_items_order',
				data: {
					order_id: order_id
				},
				cache: false,
